...
def check_md5(md5_str:str):
    """检查传入的md5字符串是否已经被处理过
        return False(md5未处理过)  Trun（已经处理过，已经有数据）    
    """
...
def get_string_md5(input_str:str,encoding="utf-8"):
    ...
    md5_obj.update(str_bytes)      #更新内容（传入即将转化为字节数组）
...
class KnowledgeBaseService(object):
    def __init__(self):
        ...
        self.spliter = RecursiveCharacterTextSplitter(
            ...
            length_function = len,                #使用Python的len函数数据长度统计的依据
        )    #文本分割器对象
    ...
    def upload_by_str(self,data:str,filename):
        ...
        self.chroma.add_texts(            #内容就加载进数据库中了
            knowledge_chunks,
...
if __name__ =='__main__':
    ...
    r= service.upload_by_str("小米","testfile")
    print(r)
